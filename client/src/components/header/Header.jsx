...
const Header = ({ resData }) => {
  ...
  return (
    <header className="relative py-3 px-10 bg-[#f7f7c0] flex justify-between items-center">
      <h2 className="text-2xl">{resData?.data?.user?.name}</h2>
      ...
      <div className="flex items-center gap-2">
        ...
        <i onClick={()=> setShow(!show)} className="fa-solid fa-angle-down text-gray-600 cursor-pointer"></i>
      </div>
      {show && (
        <div className="logout-section rounded-lg absolute top-[56px] right-[10px] bg-[beige] flex flex-col justify-center items-center overflow-hidden">
          <button className="w-full py-2 hover:bg-[#f0f0ba]" onClick={handleLogout}>Log out</button>
          <button className="py-2 px-3 hover:bg-[#f0f0ba]">
            Delete Account
          </button>
        </div>
      )}
